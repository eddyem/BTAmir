...
$W.initUtil = function() {
   ...
   source: "#ifdef GL_ES                    \n"+
    ...
    $W.util.getGLContext= function UTIL_getGLContext(canvas) {
        var gl = null;
        var type = '';
// premultipliedAlpha + alpha -> 4 alpha-blending work
// preserveDrawingBuffer -> for picking work
        try { if (!gl) {
                  type = '3d';
                  gl = canvas.getContext(type, {premultipliedAlpha: false,
        ...
        if (!!gl) { $W.debug('using ' + type, {premultipliedAlpha: false,
					preserveDrawingBuffer: true, alpha: false});}
        return gl;
    };
